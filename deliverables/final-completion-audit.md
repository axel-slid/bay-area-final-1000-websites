...
- Profile-link search status for all 1000 rows is documented in `profile-link-search-audit.csv`: 682 accepted profile links, 203 search errors/no results, and 115 searched with no high-confidence matching profile found.
- Owned-website search status for all 1000 final rows is documented in `owned-website-search-audit.csv`: 440 rows had no likely owned-site hit, 43 raw hits were reviewed but not confirmed as owned sites, and 517 rows had search-provider errors during the final audit. Confirmed owned-site removals remain recorded separately in `confirmed-owned-site-removals.csv`.
