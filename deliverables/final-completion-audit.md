...
- Profile image search status for all 1000 rows is documented in `profile-image-search-audit.csv`: 345 accepted profile images, 338 profile-linked rows searched with no safe matching image, and 317 rows with no validated profile link.
