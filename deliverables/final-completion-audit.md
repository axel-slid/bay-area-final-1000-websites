...
- Profile-link search status for all 1000 rows is documented in `profile-link-search-audit.csv`: 682 accepted profile links, 315 searched with no high-confidence matching profile found, and 3 rows with search-provider errors.
...
- Profile image search status for all 1000 rows is documented in `profile-image-search-audit.csv`: 330 accepted profile images, 322 profile-linked rows searched with no safe matching image, 30 profile-linked rows with search-provider errors, and 318 rows with no validated profile link.
